...
with DAG(
    ...
    def air_quality_wrangling():
        first_step = PythonOperator(task_id="remove_invalid_measurements",
                                    python_callable=remove_invalid_measurements)

        second_step = PythonOperator(task_id="drop_useless_columns",
                                    python_callable=drop_useless_columns)

        third_step = PythonOperator(task_id="aggregate_measurement_by_site_code_and_pollutant_type",
                                    python_callable=aggregate_measurement_by_site_code_and_pollutant_type)
        
        fourth_step = SparkSubmitOperator(task_id="merge_daily_files",
                                          conn_id="spark-conn",
