...
def rename_columns(df):
    ...
    df.to_csv(LANDING_ZONE_PATH + "analysispcwrangling.csv", sep=";", index=False)
    return df
...
def create_water_quality_measurements_sql_table():
    ...
    measurements_cleaned_data = measurements_cleaned_data[measurements_cleaned_data['code_station'].isin(stations_code)]
    if not sqlalchemy.inspect(engine).has_table("water_quality_measurements"):
        with engine.connect() as conn:
            ...
            conn.execute("ALTER TABLE water_quality_measurements ADD CONSTRAINT pk_wq_measurements PRIMARY KEY(station_code);")
...
def main():
    print(create_water_quality_station_sql_table())
